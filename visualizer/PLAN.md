...
|------|-------------|----------------------------------------------------------------|------|
| 0x01 | STATE       | pads lo7, pads hi5 · S30–S37 (8×7-bit) · swA, swB · LED (7-bit brightness) · model# · mode flags (bits0-1 mode, bit2 playing, bit3 Arp/Mel sound edit) · seq step · octave+3 · root semitone · rec slot (0x7F = idle) · NoteRec layers · NoteRec mute mask · clock src (0 int, 1 MIDI, 2 CV) · arp flags (bits0-1 latched Arp/Hold/Rec sub-state, bit2 Rec armed, bit3 melodic transport running, bits4-5 arp octave range 0-3 — base octave and range compose, so a reader wanting the span the arp covers must add them) · hold kind (0 none, 1 P0+P2, 2 rec entry, 3 layer clear, 4 layer copy, 5 rec save, 6 rec cancel, 7 P0+P1 sound edit, 8 P0+P2 in Rec, 9 kick lab step — 6 and 9 are confirm-only, no bar) · hold progress (7-bit, fraction toward that hold's threshold, per-stage for kind 1; stays 0 through the announce window at the head of each stage) · hold stage (confirms fired so far — edge-detect a rise for the confirm flash) · hold outcome (kind 3: 1 cleared, 2 empty · kind 7: 1 entered, 2 left) · seq pattern slot (0-based within the genre, the sequencer's own — S35 is behind a pickup) · pickup armed mask (bit i = S3(0+i) is behind a pickup and driving nothing yet) · 8 pickup targets (0..127, the value each armed pot must reach; only value-crossing pickups are reported — the movement-catches engage on any nudge, so there is no target to aim at) · SW1 latch (bits0-1 loaded Seq genre, bits2-3 loaded pitched scale, both panel order like swA — SW1 only takes effect in the mode you move it in, so the lever routinely disagrees with what is loaded; read these to name a genre or scale, and swA only to detect the lever moving or to mark the divergence) · arp pool (bit i = pad P(3+i) is in the arp's note pool — not derivable from the pad bits: in Hold the pool latches, so it holds notes with nothing touching them, and a re-touch takes one out) · kick preset (1-based index into the kick lab bank, `synth/kick_presets.h`; 0 = the kick pad is on whatever the kit randomizer gave it — the confirm flash that names a preset is gone in 220 ms, so the loaded one has to be readable from state) | 30 Hz while anything changed, 2 Hz keep-alive |
