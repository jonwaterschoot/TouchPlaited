...
6. **Post-v1 features** (mid-to-late July 2026): the FX send shipped on the `FX` branch (`e06a99c`→`38bdf18`, 2026-07-09), then the bigger swing — Random mode replaced by a proper Arp/Mel playmode with a layered note recorder, designed as a sketch in notes.md and implemented over 16 phases across two hardware rounds, archived once shipped in [arp-mel-plan-archive.md](arp-mel-plan-archive.md). Alongside it: CV clock in/out (S43/S40), a webapp settings/pattern editor round, per-playmode independence (sound/volume/FX/octave no longer shared across Basic Pitch/Arp/Rec), and persistent settings across power cycles via a hand-rolled QSPI journal (libDaisy's `PersistentStorage` doesn't fit a `BOOT_QSPI` app's write path, so this one's a from-scratch append-only log — see `synth/settings_journal.h`).
7. **Second notes cleanup** (2026-07-24): by this point notes.md had grown past 900 lines, mixing live reference material (control/hardware reference, per-model tables — still current, left in place) with a full stretch of resolved debugging history, two superseded design sketches, and dated implementation logs (voice-expansion/MIDI budget analysis, the MIDI implementation write-up, the FX resource analysis + implementation) whose outcomes are already shipped and captured elsewhere. That resolved material — plus the original "Playmode overhaul" and "Syncing" sketches (both already pointing at their resolutions) and a same-day-resolved visualizer UX musing — moved to [notes_archive_2026-07.md](notes_archive_2026-07.md). ROADMAP.md was rewritten around the genuinely open items surfaced in the process: filing the SSD1306 driver fix upstream (fixed locally during OLED bring-up, `62ab175` in the `lib/libDaisy` submodule, no push access), the still-undecided Chiptune engine keep/remove/improve question, and the CV-clock hardware verification pass.
