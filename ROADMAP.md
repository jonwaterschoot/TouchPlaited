...
- **OLED screen** — I2C 128×32 add-on, D11/D12 (shared with the MPR121 touch bus). Implemented 2026-07-23: `display/oled_ui.h`/`.cpp` mirrors the last-touched control (combo, function, value — engine-aware, same tables as the visualizer) from the same `TelemetryState` snapshot the webapp decodes; see notes.md "SSD1306 128×32 OLED" for the vendored driver bugs (shifted/garbled display, slow per-byte I2C sends) found and fixed along the way. Next-round idea: show a held touch combo (e.g. P0+S35, P1+S30) with a progress bar while it's building toward its threshold (bank pickup, rec entry, re-randomize hold, etc.) instead of just the settled result — same idea as the LED countdown blinks, on the screen.
